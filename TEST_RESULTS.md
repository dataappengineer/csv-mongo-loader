...
- File di test: `/home/giovanni/csv-mongo-loader/dati.csv` (5 righe)
...
| T3 | **IA — Insert Append** | `modo: IA`, stesso file CSV (5 righe) | HTTP 200, `status: SUCCESS`, `records: 5`, record aggiunti ai precedenti | ✅ |
| T4 | **IU — Insert Update (Upsert)** | `modo: IU`, `chiaveUpsert: nome`, file CSV con record già presenti | HTTP 200, `status: SUCCESS`, `records: 5`, nessun duplicato | ✅ |
| T5 | **Validazione: body incompleto** | Body JSON con campo obbligatorio mancante (`modo` assente) | HTTP 400, `status: ERROR`, messaggio con elenco campi obbligatori | ✅ |
| T6 | **Validazione: modo non valido** | `modo: XX` | HTTP 400, `status: ERROR`, `message: Il campo modo deve essere TI, IA o IU` | ✅ |
...
| # | Verifica | Risultato |
|---|----------|:---------:|
| S1 | `GET http://localhost:8080/swagger-ui/index.html` risponde con pagina HTML titolata "Swagger UI" | ✅ |
| S2 | `GET http://localhost:8080/v3/api-docs` contiene endpoint `/api/load` | ✅ |
...
- Dopo T2/T3/T4 il file CSV viene rinominato con suffisso `_loaded_yyyyMMddHHmmss.csv`
